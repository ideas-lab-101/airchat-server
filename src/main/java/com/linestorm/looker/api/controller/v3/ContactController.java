...
public class ContactController extends AppBaseController {
    ...
    public void delFriend(){
        ...
        if(r){
            ...
            new Thread(){
                public void run(){
                    Notify.dao.notify(user.getString("login_name"), friend_id+"", NotifyType.del_friend, "");
                }
            }.start();
            rest.success("操作成功！");
        }else{
    ...
    public void applyFriend(){
        ...
        if(Db.findFirst("select * from contact_relation where apply_user_id=? and confirm_user_id=? and confirmed=0", user.get("user_id"), user_id) == null){
            ...
            new Thread(){
                public void run(){
                    Notify.dao.notify(user.getString("login_name"),user_id+"", NotifyType.apply_friend, "");
    ...
    public void confirmFriend(){
        ...
        if(Db.findFirst("select id from contact_friend where user_id=? and friend_id=?"
            ...
            new Thread(){
                public void run(){
                Notify.dao.notify(user.getString("login_name"), user_id + "", NotifyType.confirm_friend, "");
